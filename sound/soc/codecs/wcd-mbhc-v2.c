					 ...
					 enum wcd_mbhc_plug_type plug_type)
{
	...
	if (plug_type == MBHC_PLUG_TYPE_HEADPHONE) {
		...
		if (mbhc->impedance_detect) {
				mbhc->mbhc_cb->compute_impedance(mbhc,
						&mbhc->zl, &mbhc->zr);
				pr_debug("%s: test ----zl:%d,zr:%d\n", __func__, mbhc->zl, mbhc->zr);
				if ((mbhc->zl > 20000) && (mbhc->zr > 20000)) {
					pr_debug("%s: test ----special accessory\n", __func__);
					/*Toggle switch back*/
					if (mbhc->mbhc_cfg->swap_gnd_mic &&
						mbhc->mbhc_cfg->swap_gnd_mic(codec)) {
						pr_debug("%s: ljl +++++US_EU gpio present,flip switch again\n"
								, __func__);
					}
					wcd_mbhc_report_plug(mbhc, 1, SND_JACK_HEADSET);
				} else {
					pr_debug("%s: headset is SND_JACK_UNSUPPORTED ;100", __func__);
					wcd_mbhc_report_plug(mbhc, 1, SND_JACK_UNSUPPORTED);
				}
			}
...
correct_plug_type:
	...
	if (plug_type == MBHC_PLUG_TYPE_HIGH_HPH &&
		...
		if (wcd_is_special_headset(mbhc)) {
			...
			pr_debug("%s: selfie stick found %d\n", __func__, plug_type);
			plug_type = MBHC_PLUG_TYPE_HEADSET;
			goto report;
