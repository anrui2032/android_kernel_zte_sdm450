...
{
	char buffer[15];
	/* dev_err(led_cdev->dev,"fangjun add for gpio led begin!!!\n"); */
	
	if (!strcmp(led_cdev->name, "red")) {
		if (value == LED_OFF) {
			dev_err(led_cdev->dev, "fangjun add for gpio red-off!!!\n");
			gpio_direction_output(STATUS_LED->red_led, 0);
		} else {
			dev_err(led_cdev->dev, "fangjun add for gpio red-on!!!\n");
			gpio_direction_output(STATUS_LED->red_led, 1);
		}
	} else if (!strcmp(led_cdev->name, "green")) {
		if (value == LED_OFF) {
			dev_err(led_cdev->dev, "fangjun add for gpio greeen-off!!!\n");
			gpio_direction_output(STATUS_LED->green_led, 0);
		} else {
			dev_err(led_cdev->dev, "fangjun add for gpio green-on!!!\n");
	...
	if (!send_info) {
		int rc = 0;
		/* dev_err(led_cdev->dev,"fangjun send_info=0!!!\n"); */
		...
		/* dev_err(led_cdev->dev,"fangjun red-set-rc=%d!!!\n", rc); */
		...
		/* dev_err(led_cdev->dev,"fangjun red-blink-rc=%d!!!\n", rc); */
		...
		/* dev_err(led_cdev->dev,"fangjun green-blink-rc=%d!!!\n", rc); */
		if (!rc)
			send_info = 1;
		else {
			dev_err(led_cdev->dev, "fangjun led send info error in %s, %s\n", __func__, led_cdev->name);
			return;
		}
	}
