	...
	unsigned int *vendor_id, unsigned int *fw_version)
{
	...
	if (chipid_in_chip == 0x00 || vendorid_in_chip == 0x00 ||
		...
		fwver_in_chip = 0x00;
		if (strnstr(tpd_fw_cdev.lcm_info, "Boe", sizeof(tpd_fw_cdev.lcm_info)) !=  NULL) {
			TPD_DMESG("%s, tpd incell structure, lcm Boe, tp Boe.\n", __func__);
			vendorid_in_chip = 0x46;
		} else
...
int synaptics_check_fw_version(struct tpd_classdev_t *cdev, unsigned char *data,
	unsigned int size, unsigned int force_upg)
{
	unsigned int chipid_in_chip, vendorid_in_chip, fwver_in_chip;
	...
	TPD_DMESG("%s: tpd Type:0x%x, Partner:0x%x, Fw_rev:0x%x, in chip\n",
		__func__, chipid_in_chip, vendorid_in_chip, fwver_in_chip);
	TPD_DMESG("%s: tpd Type:0x%x, Partner:0x%x, Fw_rev:0x%x, in new firmware\n",
		__func__, chipid_in_file, vendorid_in_file, fwver_in_file);
	...
	if (force_upg || ((vendorid_in_chip == vendorid_in_file) && (fwver_in_chip < fwver_in_file))) {
		TPD_DMESG("%s: check firmware finish, need upgrade fw!\n", __func__);
		return 0;
	}

	TPD_DMESG("%s: this chip not need upgrade firmware\n", __func__);
