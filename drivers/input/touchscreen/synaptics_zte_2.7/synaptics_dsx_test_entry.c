...
***********************************************************************/
int set_param_data(struct synaptics_test_st *stp_test)
{
	int retval = 0;

	TPD_TEST_DBG("Enter  set_param_data.");

	/* malloc ini section/key/value data space && parse them to the structure.*/
	retval = ini_get_key_data(stp_test->ini_string);
	if (retval < 0) {
	...
	TPD_TEST_DBG("end of set_param_data.");
	return 0;
}
...
static int _tpd_test_get_testparam_from_ini(struct synaptics_test_st *stp_test)
{
	...
	retval = set_param_data(stp_test);
