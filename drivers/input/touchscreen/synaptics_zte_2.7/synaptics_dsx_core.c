...
extern int synaptics_check_fw_version(struct tpd_classdev_t *cdev,
	unsigned char *data, unsigned int size, unsigned int force_upg);
...
static int tpd_check_fw_version(struct tpd_classdev_t *cdev, unsigned char *data,
	unsigned int size, unsigned int force_upg)
{
	int retval = -1;

	TPD_DMESG("%s in, fw size is:0x%x, force:0x%x.\n", __func__, size, force_upg);
	if (cdev == NULL || data == NULL || size == 0) {
		TPD_DMESG("%s firmware data is invalid.\n", __func__);
		retval = 0xff;
		goto out;
	}

	retval = tpd_require_stay_awake(cdev);
	if (retval != 0) {
		TPD_DMESG("%s require tp stay wake failed\n", __func__);
		retval = 0xff;
		goto out;
	}

	retval = synaptics_check_fw_version(cdev, data, size, force_upg);
...
static int tpd_compare_tp(struct tpd_classdev_t *cdev, unsigned char *data)
{
	...
	retval = tpd_check_fw_version(cdev, data, cdev->tp_fw.data_len, 0);
