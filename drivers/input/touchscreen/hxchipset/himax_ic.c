...
#else
	if (HX_VENDOR_ID_DIJING == g_vendor_id)
		ic_data->HX_RX_NUM				= FIX_HX_RX_NUM_DIJING;
	else if (HX_VENDOR_ID_LEAD == g_vendor_id || HX_VENDOR_ID_XINLI == g_vendor_id)
...
void himax_read_FW_ver(struct i2c_client *client)
{
	...
	/*himax_sense_off(client);*/ /* 20180117 modify for no touch while booting */
...
bool himax_ic_package_check(struct i2c_client *client)
{
	...
	for (i = 0; i < 3; i++) {
		...
		&& ((tmp_data[1] == 0x2a) || (tmp_data[1] == 0x2b))) {
			...
			} else if (HX_VENDOR_ID_LEAD == g_vendor_id) {
			I("Himax IC lead\n");
			...
			} else if (HX_VENDOR_ID_XINLI == g_vendor_id) {
			I("Himax IC xinli\n");
