	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG_READ) {
		...
		"Himax-tp\nChip type:83102\nFW version:%#x\nCfg version = 0x%2.2X\nVerdor:%s\n",
		ic_data->vendor_fw_ver, ic_data->vendor_touch_cfg_ver, HXTS_VENDOR_0_NAME);
		else if (g_vendor_id == HX_VENDOR_ID_1)
			ret += snprintf(temp_buf, 100,
		"Himax-tp\nChip type:83102\nFW version:%#x\nCfg version = 0x%2.2X\nVerdor:%s\n",
		ic_data->vendor_fw_ver, ic_data->vendor_touch_cfg_ver, HXTS_VENDOR_1_NAME);
		else if (g_vendor_id == HX_VENDOR_ID_2)
			ret += snprintf(temp_buf, 100,
		"Himax-tp\nChip type:83102\nFW version:%#x\nCfg version = 0x%2.2X\nVerdor:%s\n",
		ic_data->vendor_fw_ver, ic_data->vendor_touch_cfg_ver, HXTS_VENDOR_2_NAME);
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	cdev->ic_tpinfo.chip_model_id = 8;
