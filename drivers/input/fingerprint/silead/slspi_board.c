...
long gsl_fp_pinctrl_init(struct spidev_data *spidev)
{
	...
	if (IS_ERR(spidev->fp_pinctrl)) {
		SL_LOGD("devm_pinctrl_get failed!");
	...
	if (IS_ERR(spidev->pinctrl_state_pwractive)) {
		SL_LOGD("pinctrl_lookup_state reset_active failed!");
	...
	if (IS_ERR(spidev->pinctrl_state_suspend)) {
		SL_LOGD("pinctrl_lookup_state reset_deactive failed!");
	...
	if (IS_ERR(spidev->pinctrl_state_interrupt)) {
		SL_LOGD("pinctrl_lookup_state irq_active failed!");
...
int silead_fp_init_gpio_states(struct spidev_data *spidev)
{
	...
	if (status) {
		SL_LOGD("pinctrl_select_state pinctrl_state_interrupt failed!");
		return status;
	}
