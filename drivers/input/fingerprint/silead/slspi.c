...
{
	if (in_spi_dev == NULL) {
		SL_LOGD("input struct spidev_data is null!\n");
		return;
	}
...
static irqreturn_t spidev_irq_routing(int irq, void *dev)
{
	...
	if (g_release_esd_reset_irq == 1) {
		SL_LOGD("irq top half release esd reset irq\n");
		return IRQ_HANDLED;
	}
...
/*		}*/
		if (arg == 0) {
			...
			SL_LOGD("SPI_HW_IRQ_ENBALE error arg\n");
		}
		break;
...
/*add by thomas for int begin*/
	case SPI_GET_KERNEL_INT_INFO:
		...
		if (copy_to_user((unsigned int __user *)arg, int_count_ptr, nr_cpu_ids)) {
			SL_LOGD("copy_to_user int_count_ptr failed");
			return -EFAULT;
		}
		break;
...
static int silead_fp_request_gpios(struct spidev_data *spidev)
{
	...
	if (gpio_is_valid(spidev->hw_reset_gpio)) {
		...
		if (status) {
			SL_LOGD("devm_gpio_request reset_gpio failed! status = %d", status);
			return -EFAULT;
		}
	...
	if (gpio_is_valid(spidev->hw_int_gpio)) {
		...
		if (status) {
			SL_LOGD("devm_gpio_request_one int_gpio failed! status = %d", status);
			return -EFAULT;
		}
...
static int sl_reg_key_kernel(struct spidev_data *spidev)
{
	...
	if (IS_ERR(spidev->input)) {
		SL_LOGD("Failed to allocate input device.");
	...
	if (input_register_device(spidev->input)) {
		SL_LOGD("Failed to register input device.");
	}
	return 0;
}
#endif
...
static int	spidev_probe(struct GSL_DEV_SEL_device *spi)
{
	...
	if (!spidev) {
		SL_LOGD("no enough mem for spidev!");
		return -ENOMEM;
	}
	spidev->spi = spi;
	...
	if (ret) {
		SL_LOGD("gsl_fp_pinctrl_init failed!");
		kfree(spidev);
		return -EFAULT;
	}
	...
	if (status) {
		SL_LOGD("silead_fp_request_gpios failed!");
		kfree(spidev);
		return -EFAULT;
	}
	status = silead_fp_init_gpio_states(spidev);
	if (status) {
		SL_LOGD("silead_fp_init_gpio_states failed!");
		kfree(spidev);
		return -EFAULT;
	}
#ifndef SLPT
	...
	if (strncmp(tmp, "61", 2)) {
		SL_LOGD("the chip is not for silead\n");
...
static int __init spidev_init(void)
{
	...
	if (is_silead_fp()) {
		pr_info("<HW> Silead fingerprint hw detected\n");
	} else {
		pr_info("<HW> not Silead fingerprint hw, skip\n");
		return -EFAULT;
	}
#endif
	...
	if (status < 0) {
		SL_LOGD("GSL_DEV_SEL_register_driver failed!");
