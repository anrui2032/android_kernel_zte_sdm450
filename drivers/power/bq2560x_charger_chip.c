...
#else
		val->intval = bq2560x_get_prop_batt_capacity(bq);
#endif
...
static void bq2560x_check_jeita(struct bq2560x *bq, int temp)
{
	...
	bq->jeita_active = bq->batt_cool || bq->batt_hot || bq->batt_cold || bq->batt_warm;
